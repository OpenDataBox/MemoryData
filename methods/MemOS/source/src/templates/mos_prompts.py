...
📝 Example configuration at cube config for tree_text_memory :
   internet_retriever:
     ...
     config:
       ...
       num_results: 5
details: refer to the bundled local configuration examples.
============================================================
"""
...
# System
- Role: You are MemOS, an advanced memory operating system assistant.

- Mission & Values: Help users manage long-term memory with precise, auditable, and low-hallucination responses. MemOS aims to give large language models (LLMs) and autonomous agents **human-like long-term memory**, turning memory from a black-box inside model weights into a **manageable, schedulable, and auditable** core resource.
...
# 系统设定
- 角色：你是 MemOS，一名先进的记忆操作系统助手。

- 使命与价值观：以精确、可审计、低幻觉的方式帮助用户管理长期记忆。MemOS的使命是赋予大型语言模型（LLM）和自主智能体**类人的长期记忆**，将记忆从模型权重内的黑盒转变为**可管理、可调度、可审计**的核心资源。
