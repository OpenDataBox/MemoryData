...
from memos.embedders.base import BaseEmbedder, _truncate_text_to_tokens
...
class UniversalAPIEmbedder(BaseEmbedder):
    ...
    def embed(self, texts: list[str]) -> list[list[float]]:
        ...
        if self.provider == "openai" or self.provider == "azure":
            try:
                async def _create_embeddings(input_texts: list[str]):
                ...
                try:
                    response = asyncio.run(asyncio.wait_for(_create_embeddings(texts), timeout=timeout))
                except Exception as exc:
                    if not _is_embedding_context_length_error(exc):
                        raise
                    ...
                    retry_budget = max(256, int(max_tokens) - 1024)
                    ...
                    logger.warning(
                        "Embeddings request exceeded token budget; retrying with tighter limit=%s",
                        retry_budget,
                    )
                    response = asyncio.run(
                        asyncio.wait_for(_create_embeddings(retry_texts), timeout=timeout)
                    )
                ...
                if self.use_backup_client:
                    ...
                    try:
                        async def _create_embeddings_backup(input_texts: list[str]):
                        ...
                        try:
                            response = asyncio.run(
                                asyncio.wait_for(
                                    _create_embeddings_backup(texts),
                                    timeout=timeout,
                                )
                            )
                        except Exception as backup_exc:
                            if not _is_embedding_context_length_error(backup_exc):
                                raise
                            ...
                            model_name = getattr(
                                ...
                                "backup_model_name_or_path",
                                None,
                            ) or getattr(self.config, "model_name_or_path", None)
                            retry_texts = [
                                _truncate_text_to_tokens(
                                    text,
                                    retry_budget,
                            ...
                            logger.warning(
                                "Backup embeddings request exceeded token budget; retrying with tighter limit=%s",
                                retry_budget,
                            )
                            response = asyncio.run(
                                asyncio.wait_for(
                                    _create_embeddings_backup(retry_texts),
                                    timeout=timeout,
                                )
                            )
                        logger.info(
