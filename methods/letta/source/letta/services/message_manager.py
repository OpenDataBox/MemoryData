        ...
        with self.session_maker() as session:
            ...
            if query_text:
                if session.bind.dialect.name == "sqlite":
                    ...
                    # SQLite lacks Postgres' json_array_elements(...) helper. Fall
                    # back to a simple text/content match so local Letta runs can
                    # still use conversation_search without requiring Postgres.
                    query = query.filter(
                        or_(
                            MessageModel.text.ilike(like_pattern),
                            cast(MessageModel.content, String).ilike(like_pattern),
                        )
                    )
                else:
