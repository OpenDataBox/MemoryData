...
def get_frontend_download_info() -> Tuple[str, str]:
    """
    Get the configured archive URL and version for frontend assets.
    The archive URL is provided externally to avoid hardcoding a public repository path.
    """
    ...
    download_url_template = os.environ.get("COGNEE_FRONTEND_ARCHIVE_URL")
    if not download_url_template:
        raise RuntimeError(
            "COGNEE_FRONTEND_ARCHIVE_URL is not configured for frontend asset downloads."
        )
    download_url = download_url_template.format(version=clean_version)
...
def download_frontend_assets(force: bool = False) -> bool:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            # Download the configured frontend archive for the current version
            logger.info(f"Downloading cognee frontend archive for version {version.replace('-local', '')}...")
