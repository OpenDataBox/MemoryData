...
class Model:
    def __init__(
        ...
        self._patch_memo_rag_beacon_output_device_alignment()
    ...
    def _load_model_with_fallback(self):
        try:
            ...
            logger.warning(
                "GPU OOM while loading %s; retrying on CPU.",
                self.model_name_or_path,
            )
            if torch.cuda.is_available():
                torch.cuda.empty_cache()

            cpu_model_kwargs = dict(self.model_kwargs)
            cpu_model_kwargs["device_map"] = "cpu"
            cpu_model_kwargs["attn_implementation"] = None
            cpu_model_kwargs["torch_dtype"] = torch.float32
            cpu_model_kwargs.pop("quantization_config", None)
    ...
    def _patch_memo_rag_beacon_output_device_alignment(self):
        """MemoRAG beacon checkpoints can emit CPU beacon indices while logits live on CUDA.

        Patch the loaded model's memory.output method so masking happens on the
        same device as the logits tensor.
        """
        memory_module = getattr(self.model, "memory", None)
        if memory_module is None or getattr(memory_module, "_mad_device_patch", False):
            return

        def patched_output(beacon_self, model_outputs):
            if beacon_self.batch_loss is not None:
                loss = beacon_self.batch_loss.sum() / beacon_self.valid_token_num.sum()
                batch_loss = beacon_self.batch_loss / beacon_self.valid_token_num
                if (beacon_self.valid_token_num == 0).any():
                    batch_loss = batch_loss.masked_fill(beacon_self.valid_token_num == 0, 0.0)
                model_outputs["loss"] = loss
                model_outputs["batch_loss"] = batch_loss

            beacon_size = beacon_self.all_beacon_sizes[-1]
            if beacon_size > 0:
                logits = model_outputs["logits"]
                beacon_indices = beacon_self.beacon_indices[-logits.shape[1]:]
                if beacon_indices.device != logits.device:
                    beacon_indices = beacon_indices.to(logits.device)
                model_outputs["logits"] = logits[:, beacon_indices == 0]

            return model_outputs

        memory_module.output = MethodType(patched_output, memory_module)
        memory_module._mad_device_patch = True
...
class MemoRAG:
    ...
    def _retrieve(self, retrieval_query):
        ...
        for per_query_indices in topk_indices:
            for index in per_query_indices.tolist():
                if index < 0 or index in seen_indices:
                    continue
                seen_indices.add(index)
                ordered_unique_indices.append(index)
                if len(ordered_unique_indices) >= self.retriever.hits:
                    return [self.retrieval_corpus[i].strip() for i in ordered_unique_indices]
