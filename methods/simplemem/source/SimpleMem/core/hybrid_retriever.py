...
class HybridRetriever:
    ...
    def _default_query_analysis(self, query: str) -> Dict[str, Any]:
        """Fallback shape expected by lexical and symbolic retrieval."""
        return {
            "keywords": [query],
            "persons": [],
    ...
    def _normalize_query_analysis(self, analysis: Any, query: str) -> Dict[str, Any]:
        ...
        for key in ("time_expression", "location"):
            value = normalized.get(key)
...
"""
        ...
        for attempt in range(max_retries):
            try:
                ...
                return self._normalize_query_analysis(analysis, query)
        ...
        try:
            ...
            if isinstance(result, list):
                result = {
                    ...
                    "relationships": [],
                    "minimal_queries_needed": max(1, len(result)),
                }
            if not isinstance(result, dict):
                raise ValueError("Information requirements analysis returned non-dict JSON.")
            if "required_info" not in result:
                result["required_info"] = []
            elif not isinstance(result["required_info"], list):
                result["required_info"] = [result["required_info"]]
            return result
