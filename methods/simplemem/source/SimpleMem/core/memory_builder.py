...
class MemoryBuilder:
    ...
    def __init__(
        ...
        self.context_window = int(getattr(config, 'LLM_CONTEXT_LENGTH', 32768))
        self.output_reserve = int(getattr(config, 'LLM_OUTPUT_RESERVE', 1024))
        self.tokenizer = load_local_hf_tokenizer(getattr(config, 'LLM_MODEL', None)) or ApproximateTokenizer()
    ...
    def _is_context_limit_error(self, exc: Exception) -> bool:
        """Detect model context window failures from OpenAI-compatible errors."""
    ...
    def _generate_memory_entries(
        ...
        # Build context
        context = ""
        if include_reference_context and self.previous_entries:
        ...
        max_prompt_tokens = max(256, self.context_window - self.output_reserve)
        if prompt_token_count > max_prompt_tokens and len(dialogues) > 1:
            split_index = max(1, len(dialogues) // 2)
            print(
                "Memory extraction prompt estimated to exceed the model context window; "
                f"pre-splitting {len(dialogues)} dialogues into "
                f"{split_index} + {len(dialogues) - split_index}."
            )
            first_entries = self._generate_memory_entries(
                dialogues[:split_index],
                include_reference_context=include_reference_context,
            )
            second_entries = self._generate_memory_entries(
                dialogues[split_index:],
                include_reference_context=include_reference_context,
            )
        ...
        if last_exception and self._is_context_limit_error(last_exception):
            if include_reference_context and context:
                print(
                    "Memory extraction prompt exceeded the model context window; "
                    "retrying without previous-window reference context."
                )
                return self._generate_memory_entries(
                    dialogues,
            ...
            if len(dialogues) > 1:
                split_index = max(1, len(dialogues) // 2)
                print(
                    "Memory extraction prompt still exceeded the model context window; "
                    f"splitting {len(dialogues)} dialogues into "
                    f"{split_index} + {len(dialogues) - split_index}."
                )
                first_entries = self._generate_memory_entries(
                    dialogues[:split_index],
        ...
        print(f"All {max_retries} attempts failed to parse LLM response: {last_exception}")
        print(f"Raw response: {response[:500] if 'response' in locals() else 'No response'}")
        return []

    def _count_prompt_tokens(self, prompt: str) -> int:
        system_prompt = (
            "You are a professional information extraction assistant, skilled at extracting "
            "structured, unambiguous information from conversations. You must output valid JSON format."
        )
        system_tokens = len(self.tokenizer.encode(system_prompt, disallowed_special=()))
        prompt_tokens = len(self.tokenizer.encode(prompt, disallowed_special=()))
...
"""
    ...
    def _parse_llm_response(
        ...
        if isinstance(data, dict):
            for key in ("memory_entries", "entries", "memories", "results", "data", "items"):
                value = data.get(key)
