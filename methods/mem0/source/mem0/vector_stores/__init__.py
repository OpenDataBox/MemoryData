...
class Weaviate(VectorStoreBase):
    def __init__(
        ...
        try:
            ...
            raise ImportError(
                "The 'weaviate' library is required only when using the Weaviate vector store."
            ) from exc

        self._wvcc = wvcc
        self._get_valid_uuid = weaviate.util.get_valid_uuid
    ...
    def create_col(self, vector_size, distance="cosine"):
        ...
        properties = [
            self._wvcc.Property(name="ids", data_type=self._wvcc.DataType.TEXT),
            self._wvcc.Property(name="hash", data_type=self._wvcc.DataType.TEXT),
            self._wvcc.Property(
                name="metadata",
                data_type=self._wvcc.DataType.TEXT,
                description="Additional metadata",
            ),
            self._wvcc.Property(name="data", data_type=self._wvcc.DataType.TEXT),
            self._wvcc.Property(name="created_at", data_type=self._wvcc.DataType.TEXT),
            self._wvcc.Property(name="category", data_type=self._wvcc.DataType.TEXT),
            self._wvcc.Property(name="updated_at", data_type=self._wvcc.DataType.TEXT),
            self._wvcc.Property(name="user_id", data_type=self._wvcc.DataType.TEXT),
            self._wvcc.Property(name="agent_id", data_type=self._wvcc.DataType.TEXT),
            self._wvcc.Property(name="run_id", data_type=self._wvcc.DataType.TEXT),
        ]

        vectorizer_config = self._wvcc.Configure.Vectorizer.none()
        vector_index_config = self._wvcc.Configure.VectorIndex.hnsw()
    ...
    def insert(self, vectors, payloads=None, ids=None):
        ...
        with self.client.batch.fixed_size(batch_size=100) as batch:
            for idx, vector in enumerate(vectors):
                ...
                object_id = self._get_valid_uuid(object_id)
    ...
    def get(self, vector_id):
        ...
        """
        vector_id = self._get_valid_uuid(vector_id)
        collection = self.client.collections.get(str(self.collection_name))
