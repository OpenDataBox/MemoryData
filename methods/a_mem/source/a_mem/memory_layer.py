...
class OpenAIEmbeddingEncoder:
    """Minimal OpenAI-compatible embedding wrapper with a SentenceTransformer-like API."""
    ...
    def get_completion(
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(
                    "SGLang server returned status %s: %s",
            ...
            logger.warning("SGLang completion error: %s", e)
            empty_response = self._generate_empty_response(response_format)
            return json.dumps(empty_response)
...
class MemoryNote:
    ...
    def analyze_content(content: str, llm_controller: LLMController) -> Dict:            
        ...
        try:
            ...
            try:
                if response is None:
                    raise ValueError("LLM returned None for content analysis")
                response = re.sub(r'^```json\s*|\s*```$', '', response, flags=re.MULTILINE).strip()
                analysis = json.loads(response)
            except Exception as e:
                logger.warning("A-MEM analyze_content JSON parsing failed: %s", e)
                analysis = {
                    "keywords": [],
                    "context": "General",
                    "tags": []
            ...
            logger.warning("A-MEM analyze_content failed; falling back to defaults: %s", e)
            return {
                "keywords": [],
                "context": "General",
...
class SimpleEmbeddingRetriever:
    ...
    def __init__(self,
        ...
        """
        self.model_name = model_name
        self.provider = provider or "sentence_transformers"
        self.api_base = api_base
        self.api_key = api_key
        if self.provider in {"openai", "openai_compatible"}:
            self.model = OpenAIEmbeddingEncoder(model_name, api_key=api_key, api_base=api_base)
        else:
            self.model = SentenceTransformer(model_name)
        self.corpus = []
        self.embeddings = None
    ...
    def load(self, retriever_cache_file: str, retriever_cache_embeddings_file: str):
        ...
        if os.path.exists(retriever_cache_file):
            ...
            with open(retriever_cache_file, 'rb') as f:
                ...
                self.api_base = state.get('api_base', self.api_base)
    ...
    @classmethod
    def load_from_local_memory(cls,
                               memories: Dict,
                               model_name: str,
                               provider: Optional[str] = None,
                               api_key: Optional[str] = None,
                               api_base: Optional[str] = None) -> 'SimpleEmbeddingRetriever':
        """Load retriever state from memory"""
        ...
        retriever = cls(model_name, provider=provider, api_key=api_key, api_base=api_base)
        retriever.add_documents(all_docs)
        return retriever
...
class AgenticMemorySystem:
    ...
    def __init__(self, 
        ...
        self.embedding_provider = embedding_provider or "sentence_transformers"
    ...
    def process_memory(self, note: MemoryNote) -> bool:
        ...
        response = self.llm_controller.llm.get_completion(
            prompt_memory,response_format={"type": "json_schema", "json_schema": {
                        ...
                        "strict": True
                    }}, max_tokens=640, timeout_s=45.0
        )
        try:
            if response is None:
                raise ValueError("LLM returned None for process_memory")
            # Clean the response in case there's extra text
            response_cleaned = response.strip()
            # Try to find JSON content if wrapped in other text
            if not response_cleaned.startswith('{'):
            ...
            logger.warning("A-MEM process_memory JSON parsing failed: %s", e)
            ...
            for neighbor in neighborhood:
                if isinstance(neighbor, int):
                    ...
                    if neighbor_memory is None:
                        continue
                memory_str += "talk start time:" + neighbor_memory.timestamp + "memory content: " + neighbor_memory.content + "memory context: " + neighbor_memory.context + "memory keywords: " + str(neighbor_memory.keywords) + "memory tags: " + str(neighbor_memory.tags) + "\n"
                if j >=k:
                    break
                j += 1
        return memory_str
