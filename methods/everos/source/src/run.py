...
def ensure_supported_python():
    ...
    logger.error(
        "❌ Unsupported Python version: %s.%s.%s. EverOS currently requires Python 3.12.x.",
        sys.version_info.major,
        sys.version_info.minor,
        sys.version_info.micro,
    )
    logger.error(
        "💡 Recommended startup flow: cd methods/everos/source && uv sync && uv run python src/run.py --port 1995"
    )
    sys.exit(1)
