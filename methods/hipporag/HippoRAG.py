...
class HippoRAG:
        ...
        for chunk_key, triples in tqdm(zip(chunk_ids, chunk_triples)):
            ...
            if chunk_key not in current_graph_nodes:
                for triple in triples:
                    ...
                    node_key = compute_mdhash_id(content=triple[0].lower(), prefix=("entity-"))
                    node_2_key = compute_mdhash_id(content=triple[2].lower(), prefix=("entity-"))
        ...
        for idx, chunk_key in tqdm(enumerate(chunk_ids)):
            ...
            if chunk_key not in current_graph_nodes:
                for chunk_ent in chunk_triple_entities[idx]:
                    node_key = compute_mdhash_id(chunk_ent.lower(), prefix="entity-")
    ...
    def save_openie_results(self, all_openie_info: List[dict]):
        ...
        if len(all_openie_info) > 0:
            avg_ent_chars = round(sum_phrase_chars / num_phrases, 4) if num_phrases > 0 else 0.0
            avg_ent_words = round(sum_phrase_words / num_phrases, 4) if num_phrases > 0 else 0.0
    ...
    def get_fact_scores(self, query: str) -> np.ndarray:
        ...
        if self.fact_embeddings is None or getattr(self.fact_embeddings, "size", 0) == 0:
            return np.array([])
    ...
    def graph_search_with_fact_entities(self, query: str,
        ...
        if sum(node_weights) <= 0:
            logger.warning(f"No phrases found in the graph for the given facts; falling back to dense passage retrieval: {top_k_facts}")
