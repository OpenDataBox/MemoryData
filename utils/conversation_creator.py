...
from benchmark.memoryagentbench.loader import load_eval_data
...
class ConversationCreator:
    ...
    def _process_dataset_item(self, dataset_item):
        ...
        if "context_chunks" in dataset_item:
            context_payload = dataset_item["context_chunks"]
            context_length = sum(len(str(chunk.get("text", ""))) for chunk in context_payload)
            assert context_length > 2000, f"Context too short: {context_length} characters"
        else:
            context_payload = dataset_item["context"]
            assert len(context_payload) > 2000, f"Context too short: {len(context_payload)} characters"

        # Extract all non-context fields for question generation
        question_data = {
            key: value
            for key, value in dataset_item.items()
            if key not in {"context", "context_chunks"}
        }

        # Create query-answer pairs from the question data
        qa_pairs = self._create_query_answer_pairs(question_data)
        
        return context_payload, qa_pairs
    ...
    def _create_single_qa_pair(self, question_data, question, answer, question_index):
        ...
        Returns:
            tuple: (formatted_query, answer, qa_pair_id[, eval_metadata])
        """
        ...
        eval_metadata = qa_metadata.get('eval_metadata')

        return (
            (formatted_query, answer, qa_pair_id, eval_metadata)
            if eval_metadata is not None
            else (formatted_query, answer, qa_pair_id)
        )
    ...
    def _create_qa_metadata(self, question_data, question, answer, question_index):
        ...
        indexed_fields = ['question_dates', 'question_types', 'question_ids', 'previous_events', 'qa_pair_ids', 'eval_metadata']
    ...
    def _validate_chunks_structure(self, chunks):
        ...
        assert isinstance(chunks[0][0], (str, dict)), "Each chunk should be a string or structured chunk dict"
        if isinstance(chunks[0][0], dict):
            assert "text" in chunks[0][0], "Structured chunks must include a text field"
    ...
    def _validate_qa_structure(self, query_and_answers):
        ...
        # Each QA pair should be a tuple of (query, answer, qa_pair_id[, eval_metadata])
        if len(query_and_answers[0]) > 0:
            assert len(query_and_answers[0][0]) in {3, 4}, "Each QA pair should be a tuple of (query, answer, qa_pair_id[, eval_metadata])"
